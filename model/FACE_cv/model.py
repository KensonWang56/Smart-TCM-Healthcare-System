...
class FaceRecognitionModel:
    ...
    def recognize_face(self, image, threshold=0.6) -> List[Tuple[str, float]]:
        """
        识别图片中的人脸
        ...
        Returns:
            List[Tuple[str, float]]: 识别结果列表，每个元素为(名字, 相似度)
        """
        try:
            # 提取人脸特征
            face_encoding = self.extract_face_encoding(image)
            ...
            if max_similarity >= threshold:
                return [(self.known_face_names[best_match_index], max_similarity)]
            else:
                return [("未知", max_similarity)]
